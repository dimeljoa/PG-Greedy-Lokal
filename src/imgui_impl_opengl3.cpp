...
// Forward declarations needed by RenderDrawData
...
#elif defined(IMGUI_IMPL_OPENGL_LOADER_GLAD2)
    if (gladLoadGL((GLADloadfunc)glfwGetProcAddress) == 0) // Note: GLAD2 doesn't have a loader of its own. We are using the GLFW one.
    {
        fprintf(stderr, "Failed to initialize OpenGL loader!\n");
        return false;
    }
